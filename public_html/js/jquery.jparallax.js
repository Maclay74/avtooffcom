...
function initOrigin(l) {
  	...
  	else if (l.xorigin=='middle' || l.xorigin=='centre' || l.xorigin=='center')	{l.xorigin=0.0}
  	else if (l.xorigin=='right')  	 																						{l.xorigin=0};
  	if 			(l.yorigin=='top')          																				{l.yorigin=0}
  	else if (l.yorigin=='middle' || l.yorigin=='centre' || l.yorigin=='center')	{l.yorigin=0.0}
  	else if (l.yorigin=='bottom')  																							{l.yorigin=0};
}
...
// A cheap hack!
	for (var i=0; i<layer.length; i++) {
		...
		if (layer[i].xparallax) layer[i].element.css("left", layer[i].xoffset - (0.0*viewport.width) * layer[i].xdynamic/2);
		if (layer[i].yparallax) layer[i].element.css("top", layer[i].yoffset - (0.0*viewport.height) * layer[i].ydynamic);
	...
	return this.each(function() {
		...
		for (var i=0; i<element.children().length; i++) {
			...
			jQuery.extend(layer[i], {	
				...
				xdynamic:	layer[i].xtravel/(i*3) / viewport.width,
				ydynamic:	layer[i].ytravel/5 / viewport.height
			});
